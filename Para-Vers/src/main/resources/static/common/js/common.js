...
function connectAndSubscribeMQ(role, $http, $rootScope, $filter,mqConsumeCallback,mqProduceCallback){
    var ws = new WebSocket('ws://localhost:61624', 'stomp');
    ...
    var on_connect = function(x) {
    	var requisition = $filter('translate')('REQUISITION');
...
function doDisplayLoadError(result,$rootScope,$filter,$http,showToastr,errorType){
	...
	if(status === 500){
		if(data.message.indexOf("can not establish connection with Virtualize server") > -1 || 
				data.message.indexOf("could not execute query") > -1){
			handleJDBCErrorMessage($rootScope,$filter,data.data,errorType,true);
		}else{
			handleRestpointErrorMessage($rootScope,$http,$filter,errorType,url,showToastr);
		}
	}else if(status === 404){
		var errorMessage;
		if(data.message === undefined){
			if(data.indexOf("x-forwarded-prefix: /proxy") > -1){
				handleRestpointErrorMessage($rootScope,$http,$filter,errorType,url,showToastr);
			}else{
				errorMessage = data;
				handleMessageAccordingToType($rootScope,errorMessage,errorType,showToastr);
			}
		}else{
			errorMessage = $filter('translate')('REST_ENDPOINT_ERROR') + data.path + ' ' + $filter('translate')('UNAVAILABLE_TO_CONNECT');
			handleMessageAccordingToType($rootScope,$filter,errorMessage,errorType,showToastr,true);
		}
	}
}

function handleRestpointErrorMessage($rootScope,$http,$filter,errorType,url,showToastr){
	$http({
	    method: 'GET',
	    url: '/v1/demoAdmin/currentPreferences'
	}).then(function successCallback(response) {
		...
		var errorMessage = $filter('translate')('REST_ENDPOINT_ERROR') + url + ' ' + $filter('translate')('UNAVAILABLE_TO_CONNECT');
		handleMessageAccordingToType($rootScope,$filter,errorMessage,errorType,showToastr,true);
	}, function errorCallback(response) {});
}
...
function handleJDBCErrorMessage($rootScope,$filter,url,errorType,showToastr){
	...
	var errorMessage;
	if(showToastr && $.inArray(tostrErrorMessage,errorMessages) < 0 && url !== null){
