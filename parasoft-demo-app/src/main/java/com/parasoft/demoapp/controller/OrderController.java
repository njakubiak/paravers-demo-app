...
@Tag(name = "orders")
@Controller
@RequestMapping({"/v1/orders", "/proxy/v1/orders"})
public class OrderController {
	...
	@ApiResponse(responseCode = "200", description = "Submit a new order successfully.")
	@ApiResponse(responseCode = "400", description = "Invalid request payload.",
				content = {@Content(schema = @Schema(hidden = true))})
	@ApiResponse(responseCode = "403", description = "The user does not have permission to submit a new order.",
			content = {@Content(schema = @Schema(hidden = true)) })
	...
	@ApiResponse(responseCode = "200", description = "Return an order with corresponding order number.")
	@ApiResponse(responseCode = "400", description = "Invalid request parameter.",
				content = {@Content(schema = @Schema(hidden = true))})
	@ApiResponse(responseCode = "403",description = "The user does not have permission to get an order by order number.",
				content = {@Content(schema = @Schema(hidden = true)) })
	...
	@ApiResponse(responseCode = "200", description = "Return the modified order with corresponding order number.")
	@ApiResponse(responseCode = "400", description = "Invalid request parameter.",
			content = {@Content(schema = @Schema(hidden = true))})
	@ApiResponse(responseCode = "403",description = "The user does not have permission to change status.",
			content = {@Content(schema = @Schema(hidden = true))})
	@ApiResponse(responseCode = "404", description = "No order with corresponding order number.",
			content = {@Content(schema = @Schema(hidden = true))})
	...
	//					description = "The user does not have permission to get all orders.",
	//					content = {@Content(schema = @Schema(hidden = true)) }),
	//	})
	...
	public ResponseResult<PageInfo<OrderEntity>> showAllOrders(Authentication auth,
			 @ParameterObject
			 @PageableDefault(sort = "orderNumber", direction = Sort.Direction.ASC, size = Integer.MAX_VALUE)
					 			Pageable pageable)
