...
public class GlobalPreferencesDTO {
    ...
    @Schema(description = "Only works when useParasoftJDBCProxy filed is enabled. Default value is http://localhost:9080 if this field is not set.")
    private String parasoftVirtualizeServerUrl;

    @Schema(description = "Only works when useParasoftJDBCProxy filed is enabled. Default value is /virtualDb if this field is not set.<br/>" +
                          "The path must start with '/' and cannot contain with any characters but 0-9, a-z, A-Z, - and _ .")
    private String parasoftVirtualizeServerPath;

    @Schema(description = "Only works when useParasoftJDBCProxy filed is enabled. Default value is /pda if this field is not set.<br/>" +
                          "The group id can not contain with any characters but 0-9, a-z, A-Z, - and _ .")
